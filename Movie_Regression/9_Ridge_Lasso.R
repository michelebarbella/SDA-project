...
library(glmnet)
attach(mov)
x = model.matrix(linear_model, mov)[,-1] #[-1] means no intercept
...
dim(coef(ridge.mod)) # 12 coefficients, 100 lambda values
predictors = dim(coef(ridge.mod))[1]
...
predict(ridge.mod,s=50,type="coefficients")[1:predictors,] # s = lambda value, predict of the coefficients
...
lm(y~x, subset=train)
predict(ridge.mod,s=0,exact=T,type="coefficients",x=x[train,],y=y[train])[1:predictors,]
...
bestlam # the best lambda is 235.3432
log(bestlam) # log value of previous lambda is 5.461045

# Prediction of the model with the best value of lambda
ridge.pred=predict(ridge.mod,s=bestlam ,newx=x[test,])
mean((ridge.pred-y.test)^2) # MSE = 8692656

# Prediction of the coefficients with the best value of lambda
out=glmnet(x,y,alpha=0)
predict(out,type="coefficients",s=bestlam)[1:predictors,]
# As expected, none of the coefficients is zero. Ridge regression doesn't perform variable selection.
...
dev.new(); plot(cv.out)
bestlam=cv.out$lambda.min; print(bestlam); print(log(bestlam)) # the best lambda is 4.507449 in log

lasso.pred=predict(lasso.mod,s=bestlam ,newx=x[test,])
mean((lasso.pred-y.test)^2) # 8774099 slighly larger than ridge
...
mean((lasso.pred-y.test)^2) # 8714612

# However, the lasso has a substantial advantage:
# 5 of the 11 coefficient estimates is exactly zero: Lead_.Actor_Rating, 
# Lead_Actress_rating, Producer_rating, Time_taken, Twitter_hastags
out=glmnet(x,y,alpha=1,lambda=grid)
lasso.coef=predict(out,type="coefficients",s=bestlam)[1:predictors,]
lasso.coef
lasso.coef[lasso.coef!=0]
cat("Number of coefficients equal to 0:",sum(lasso.coef==0),"\n")

# Compare with OLS when only selected predictors are included. 
fit.lm=lm(Budget ~ Marketing.expense + Production.expense + Director_rating+ Critic_rating + Trailer_views +Avg_age_actors, data=mov)
coef(fit.lm)
