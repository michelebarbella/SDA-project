...
npredictors = 44
regfit.full=regsubsets(poly4_model, mov, nvmax = npredictors)
reg.summary=summary(regfit.full)

reg.summary$rsq # R2 statistic increases monotonically as more variables are included. From 36% up to 52%.
which.min(reg.summary$rss) # identify the location of the minimum in the model with 44 predictors. 
which.max(reg.summary$adjr2) # Adjusted R2 statistic is max in the model with 26 predictors.
which.min(reg.summary$bic) # BIC is min in the model with 6 predictors
which.min(reg.summary$cp) # Cp is min in the model with 16 predictors
...
## Forward
regfit.fwd=regsubsets(poly4_model,data=mov, nvmax=npredictors, method ="forward")
summary(regfit.fwd)
# We see that using forward stepwise selection, the best one-variable is recommended.

## Backward
regfit.bwd=regsubsets(poly4_model,data=mov,nvmax=npredictors, method ="backward")
summary(regfit.bwd)
...
regfit.best=regsubsets(poly4_model,data=mov[train,], nvmax = npredictors)
...
test.mat=model.matrix(poly4_model,data=mov[test,])
...
val.errors
indexmin = which.min(val.errors); val.errors[indexmin] # min MSE = 8812859
