...
library(glmnet)
attach(mov)
x = model.matrix(poly3_model, mov)[,-1] #[-1] means no intercept
...
dim(coef(ridge.mod)) # 34 coefficients, 100 lambda values
...
predict(ridge.mod,s=50,type="coefficients")[1:34,] # s = lambda value, predict of the coefficients
...
lm(y~x, subset=train)
predict(ridge.mod,s=0,exact=T,type="coefficients",x=x[train,],y=y[train])[1:34,]
...
bestlam # the best lambda is 1144.38
log(bestlam) # log value of previous lambda is 7.042618

# Prediction of the model with the best value of lambda
ridge.pred=predict(ridge.mod,s=bestlam ,newx=x[test,])
mean((ridge.pred-y.test)^2) # MSE = 8320285
...
dev.new(); plot(cv.out)
bestlam=cv.out$lambda.min; print(bestlam);print(log(bestlam)) # the best lambda is 4.972618

lasso.pred=predict(lasso.mod,s=bestlam ,newx=x[test,])
mean((lasso.pred-y.test)^2) # 8246507 slighly larger than ridge
...
mean((lasso.pred-y.test)^2) # 8226480
...
# 16 of the 34 coefficient estimates is exactly zero