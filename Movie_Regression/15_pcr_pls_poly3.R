...
pcr.fit=pcr(poly3_model, data=mov , scale=TRUE, validation ="CV")

#Data: 	X dimension: [33] 484  
#       Y dimension: [1]  484
...
# This suggests that there is a benefit in terms of reduction of dimensionality (M = 24)

# Now perform PCR on the training data and evaluate its test set performance:
set.seed (5)
x = model.matrix(poly3_model,data = mov)[,-1]
y = mov$Budget
...
pcr.fit=pcr(poly3_model, data=mov, subset=train, scale=TRUE, validation ="CV")

# Plot MSE and RMSE 
dev.new(); validationplot(pcr.fit,val.type="MSEP",legendpos = "topright") 
minPCR <- which.min(MSEP(pcr.fit)$val[1,,][-1]);
minPCR # M=10 shows the lowest CV error
...
# Setting scale=TRUE has the effect of standardizing each predictor (scale projection).
# Setting validation = 'CV' has the effect to use cross validation to rate M parameter
pls.fit=plsr(poly3_model,data = mov, scale=TRUE, validation ="CV")
summary(pls.fit)

dev.new(); validationplot(pls.fit,val.type="MSEP")
which.min(MSEP(pls.fit)$val[1,,][-1]) # M = 3

# Now perform Pls on the training data and evaluate its test set performance:
set.seed (5)
pls.fit=plsr(poly3_model,data = mov, subset=train, scale=TRUE, validation ="CV")

validationplot(pls.fit,val.type="MSEP"); 
ncomp = which.min(MSEP(pls.fit)$val[1,,][-1]); ncomp # M = 3

pls.pred=predict(pls.fit,x[test,],ncomp+1)
mean((pls.pred-y.test)^2) # --> 8481315

pls.pred=predict(pls.fit,x[test,],ncomp)
mean((pls.pred-y.test)^2) # --> 8771605

pls.pred=predict(pls.fit,x[test,],ncomp-1)
mean((pls.pred-y.test)^2) # --> 9302873
...
pls.fit=plsr(poly3_model,data = mov ,scale=TRUE,ncomp)
summary(pls.fit) # 3 comp -> 47.30


# The percentage of variance in Movie that the 3-component PLS fit explains, 47.30%,
# is quite different to that explained using the final 10-component model PCR fit, 36.44%.