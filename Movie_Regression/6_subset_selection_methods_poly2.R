...
npredictors = 22
regfit.full=regsubsets(poly2_model, mov, nvmax = npredictors)
reg.summary=summary(regfit.full)

reg.summary$rsq # R2 statistic increases monotonically as more variables are included. From 36% up to 43%.
which.min(reg.summary$rss) # identify the location of the minimum in the model with 22 predictors. 
which.max(reg.summary$adjr2) # Adjusted R2 statistic is max in the model with 15 predictors.
which.min(reg.summary$bic) # BIC is min in the model with 4 predictors
which.min(reg.summary$cp) # Cp is min in the model with 7 predictors
...
# Coefficienti associati a un modello migliore, es quello a minimo BIC ovvero a 4 variabili
...
## Forward
regfit.fwd=regsubsets(poly2_model,data=mov, nvmax=npredictors, method ="forward")
summary(regfit.fwd)
# We see that using forward stepwise selection, the best one-variable is recommended.

## Backward
regfit.bwd=regsubsets(poly2_model,data=mov,nvmax=npredictors, method ="backward")
summary(regfit.bwd)
...
regfit.best=regsubsets(poly2_model,data=mov[train,], nvmax = npredictors)
...
test.mat=model.matrix(poly2_model,data=mov[test,])
...
val.errors
indexmin = which.min(val.errors); val.errors[indexmin] # min MSE = 9156773
