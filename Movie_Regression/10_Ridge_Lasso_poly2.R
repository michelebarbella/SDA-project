...
library(glmnet)
attach(mov)
x = model.matrix(poly2_model, mov)[,-1] #[-1] means no intercept
...
bestlam # the best lambda is 283.4717
log(bestlam) # log value of previous lambda is 5.647112

# Prediction of the model with the best value of lambda
ridge.pred=predict(ridge.mod,s=bestlam ,newx=x[test,])
mean((ridge.pred-y.test)^2) # MSE = 8719061
...
dev.new(); plot(cv.out)
bestlam=cv.out$lambda.min; print(bestlam);print(log(bestlam)) # the best lambda is 4.228348

lasso.pred=predict(lasso.mod,s=bestlam ,newx=x[test,])
mean((lasso.pred-y.test)^2) # 8721803 slighly larger than ridge
...
mean((lasso.pred-y.test)^2) # 8897057
...
# 8 of the 23 coefficient estimates is exactly zero